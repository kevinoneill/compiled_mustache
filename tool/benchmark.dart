...
const _warning = '<!-- THIS FILE IS AUTOGENERATED BY \'grind doc\'; DO NOT MODIFY -->\n\n';
...
Future _documentCompiledMustache(Map<String, Map<String, String>> results) async {
  ...
  String contents = 'compiled_mustache version $cmver';
  ...
  await _writeToDocFile('compiled_mustache-benchmarks', contents);
}

Future<String> _documentMustache4Dart(Map<String, Map<String, String>> results) async {
  ...
  String contents = 'mustache4dart version $m4dver';
  for (var n in results.keys) {
    var suite = results[n];
  ...
  String name = 'mustache4dart-benchmarks';
  await _writeToDocFile(name, contents);
  return name;
}
...
Future _documentComparison(Map<String, Map<String, String>> compiled_mustache_results, Map<String, Map<String, String>> mustache4dart_results) async {
  ...
  for (var group in _comparisons) {
    ...
    for (var n in pairs.keys) {
      ...
      contents += '\n\n### [$n](compiled_mustache-benchmarks#${n.toString().toLowerCase()}) vs [${pairs[n]}](mustache4dart-benchmarks#${pairs[n].toString().toLowerCase()})\n';
  ...
  var versions = 'compiled_mustache version $cmver  \nmustache4dart version $m4dver\n';
  var toWrite = '\n\n$versions\n\n## Results  \n**Average:** ${avgStr}x  \n**Minimum:** ${minDiffStr}x  \n**Maximum:** ${maxDiffStr}x\n' + contents;
  
  await _writeToDocFile('Comparison-of-compiled_mustache-and-mustache4dart', toWrite);
  await _updateReadme(minDiffStr, maxDiffStr);
}

Future _writeToDocFile(String name, String contents) async {
  var file = await new File('compiled_mustache.wiki/$name.md');
  if (!file.existsSync()) {
    file.createSync(recursive: true);
  }
  file.writeAsString(_warning + contents);
}

Future _updateReadme(String minDiff, String maxdiff) async {
  var file = await new File('compiled_mustache.wiki/Home.md');
  if (!file.existsSync()) {
    return;
  }
